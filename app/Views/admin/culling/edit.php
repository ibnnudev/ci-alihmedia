...
<div class="card mb-4">
    <div class="card-body">
        ...
        <form action="<?= base_url('admin/culling/update/' . $culling['id']) ?>" method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" required id="norm" name="norm">
                            ...
                            <?php foreach ($patients as $patient) : ?>
                                <option value="<?= $patient['norm'] ?>" <?php if ($culling['norm'] == $patient['norm']) : ?> selected <?php endif; ?>> <?= $patient['norm'] . ' - ' . $patient['name'] ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="diagnose" class="form-label">Diagnosa</label>
                        <input type="text" class="form-control" required id="diagnose" name="diagnose" value="<?= $culling['diagnose'] ?>">
                    </div>
                    <div class="mb-3">
                        <label for="description" class="form-label">Deskripsi</label>
                        <textarea class="form-control" required id="description" name="description" rows="3"><?= $culling['description'] ?></textarea>
                    </div>
                    <div class="mb-3">
                        ...
                        <select class="form-select" required id="scan_status" name="scan_status">
                            ...
                            <option value="1" <?php if ($culling['scan_status'] == 1) : ?> selected <?php endif; ?>>Sudah di-scan</option>
                            <option value="0" <?php if ($culling['scan_status'] == 0) : ?> selected <?php endif; ?>>Belum di-scan</option>
                        </select>
                    </div>
                    <div class="mb-3">
                        <label for="culling_date" class="form-label">Tanggal Pelestarian</label>
                        <input type="date" class="form-control" required id="culling_date" name="culling_date" value="<?= $culling['culling_date'] ?>">
                    </div>
                </div>
            </div>
